...
createDFA x = NFA {
                  ...
                  trans  =  tra }
              where
              (sta,tra) = dfareach [x] ([x],[])

dfareach :: [RE] -> ([RE],[(RE,Char,RE)]) -> ([RE],[(RE,Char,RE)])
dfareach [] p             =  p
dfareach (x:xs) (sts,tra) =  dfareach (newstates ++ xs) (newstates ++ sts, newtrans ++ tra)
    where newstates =  nubSort [ s | (_,_,s)<-newtrans ] \\ sts
          newtrans  =  [ (x,c,fuse(derive c x)) | c <- enumerateSet $ fir x ]
...
stateMinX :: Derivation -> NFA RE Char -> NFA RE Char
stateMinX d nfa =
    NFA { states = map pickMinList gord,
          start  = mf (start nfa),
          final  = ewp, 
          eps    = nubSort $ epsTrans mf (eps nfa),
          trans  = nubSort $ charTrans mf (trans nfa) }
    where
    gord            =  groupOrder (compREDer d) [ r | r<-states nfa]
    mf              =  quotientMap pickMinList gord
    newstart        =  mf $ start nfa 
...
{- computing a simulation relation, quotient by sim equivalence, remove redundant transitions/states -}
stateSimulation :: Ord a => Bool -> NFA a Char -> M.Map a [a]
stateSimulation direction nfa =
    checkStates (states nfa) simMap
    where
    simlist = makeSimulation direction nfa
    simMap  = M.fromList simlist -- initial simulation candidate
    checkStates [] sm     = sm -- found a fixpoint
    checkStates (x:ss) sm | null zs -- ok for state x
                          = checkStates ss sm
                          | otherwise
                          = checkStates (states nfa) (M.insert x ys sm)
        where
        xs = sm M.! x
        (ys,zs) = partition (\y->simulatedBy direction nfa x y sm) xs

data SimulRel a = SimulRel { eqcl :: [[a]], quotient :: M.Map a a, hasse :: [ (a,a) ] }

instance Show a => Show (SimulRel a)
    where
    show sim = "equivalence classes: " ++ show (eqcl sim) ++ "\n" ++
               "hasse diagram: " ++ displayList (showAs "<") (hasse sim)

displayList :: (a->String) -> [a] -> String
displayList _ [] = ""
displayList f [x] = f x
displayList f (x:xs) = f x ++ "," ++ displayList f xs

showAs :: Show a => String -> (a,a) -> String
showAs rel (x,y) = show x ++ rel ++ show y

computeSimul :: Ord a => NFA a Char -> SimulRel a
computeSimul nfa = phases nfa $ initSimul nfa

evalSimul :: Ord a => NFA a Char -> SimulRel a -> a -> a -> Bool
evalSimul nfa sim x y = all (\(cc,xy)-> any (\(dd,yz)->cc==dd && dagSearch (hasse sim) xy yz) yout) xout
                        where
                        xout = [ (c,quotient sim M.! xx) | (z,c,xx) <- trans nfa, z==x ]
                        yout = [ (d,quotient sim M.! yy) | (z,d,yy) <- trans nfa, z==y ]

dagSearch :: Ord a => [(a,a)] -> a -> a -> Bool
dagSearch xs x y = isNothing $ dagS x y xs
                   

dagS :: Ord a => a -> a -> [(a,a)] -> Maybe [(a,a)]
dagS b c as = dse [] as b c

dse :: Ord a => [(a,a)] -> [(a,a)] -> a -> a -> Maybe [(a,a)]
dse ys xs x y | x==y
              = Nothing
dse ys [] x y = Just ys
dse ys ((x1,y1):xs) x y
              | x==x1
              = dagS y1 y (xs++ys) >>= dagS x y
              | otherwise
              = dse ((x1,y1):ys) xs x y 

phase1 :: Ord a => NFA a Char -> SimulRel a -> SimulRel a
phase1 nfa sim = p1check (eqcl sim)
    where
    p1check [] = sim
    p1check (xs:xss) =
        case checkClass nfa sim xs of
        Nothing -> p1check xss
        Just si -> phase1 nfa si

phase2 :: Ord a => NFA a Char -> SimulRel a -> SimulRel a
phase2 nfa sim =
    case checkOrder nfa sim of
    Nothing -> sim
    Just sim2 -> phases nfa sim2

phases :: Ord a => NFA a Char -> SimulRel a -> SimulRel a
phases nfa sim = phase2 nfa $ phase1 nfa sim
    

initSimul :: Ord a => NFA a Char -> SimulRel a
initSimul nfa = SimulRel { eqcl = [ fins, nonfins ],
                           quotient = insertClass (insertClass M.empty fins) nonfins,
                           hasse = [ (head nonfins,head fins) ] }
                where
                (fins,nonfins) = partition (final nfa) (sort $ states nfa)    

checkOrder :: Ord a => NFA a Char -> SimulRel a -> Maybe (SimulRel a)
checkOrder nfa sim = listToMaybe [ sim{ hasse=newhasse x y xs } | ((x,y),xs) <- itemRest (hasse sim), not (evalSimul nfa sim x y) ]

-- currently not preserving "hasse" quality, i.e. not reduced
newhasse:: Ord a => a -> a -> [(a,a)] -> [(a,a)]
newhasse x y xs = foldr addPairRel xs (newpairs x y xs)
{-
newhasse x y [] = []
newhasse x y ((v,w):xs) | v==y
                        = (x,w) : (y,w) : newhasse x y xs
                        | w==x
                        = (v,y) : (v,x) : newhasse x y xs
                        | otherwise
                        = (v,w) : newhasse x y xs
-}

addPairRel:: Ord a => (a,a) -> [(a,a)] -> [(a,a)]
addPairRel (x,y) xs | dagSearch xs x y
                    = xs
                    | otherwise
                    = (x,y):xs

newpairs:: Ord a => a -> a -> [(a,a)] -> [(a,a)]
newpairs x y [] = []
newpairs x y ((v,w):xs) | v==y
                        = (x,w) : newpairs x y xs
                        | w==x
                        = (v,y) : newpairs x y xs
                        | otherwise
...
checkClass :: Ord a => NFA a Char -> SimulRel a -> [a] -> Maybe (SimulRel a)
checkClass nfa sim [x]    =  Nothing  -- cannot anything with singletons, this eq is not necessary
checkClass nfa sim (x:xs) |  not (null fs)
                          =  Just $ SimulRel { eqcl=aux (x:ts) fs,
                                               quotient=insertClass (quotient sim) fs,
                                               hasse=(f,x):[(a,f)|(a,_)<-rx] ++ orx }
                          |  not (null us)
                          =  Just $ SimulRel { eqcl=aux (x:gs) us,
                                               quotient=insertClass (quotient sim) us,
                                               hasse=(x,u):[(u,b)|(_,b)<-lx] ++ olx } 
                          |  otherwise
                          =  Nothing
    where
    (ts,fs)=partition (evalSimul nfa sim x) xs
    f      =head fs
    (gs,us)=partition (\z->evalSimul nfa sim z x) xs
    u      =head us
    aux ns1 ns2 = ns1:ns2:(eqcl sim \\ [x:xs])
    (lx,olx) = partition (\(a,b)->a==x) (hasse sim)
    (rx,orx) = partition (\(a,b)->b==x) (hasse sim)

insertClass :: Ord a => M.Map a a -> [a] -> M.Map a a
insertClass m xs = M.union (M.fromList (zip xs (repeat (head xs)))) m

          
    


-- checking that a simul is a smul
simulatedBy :: Ord a => Bool -> NFA a Char -> a -> a -> M.Map a [a] -> Bool
simulatedBy direction nfa a b sm | a==b = True --shortCut
simulatedBy direction nfa a b sm = all (\(c,s)->any (\(b,t)->c==b && elem t (sm M.! s)) bs) as
                                   where
                                   as = charfunPar direction nfa a
                                   bs = charfunPar direction nfa b

-- initial candidate for a simulation
makeSimulation :: Ord a => Bool -> NFA a Char -> [(a,[a])]
makeSimulation direction nfa = 
    [ (x,[ y | y<- ss, rawSimul x y ]) | x<- ss ]
    where
    ss = sort (states nfa)
    fis | direction
        = nubSort [ (c,x) | (x,c,_) <- trans nfa ]
        | otherwise
        = nubSort [ (c,x) | (_,c,x) <- trans nfa ]
    charmap = M.fromList (pairQuotient fis)
    chars = nubSort [ c | (_,c,_) <- trans nfa ]
    rawSimul a b | direction
                 = (final nfa a ===> final nfa b) && charCheck chars a b
                 | otherwise
                 = ((a==start nfa) ===> (a==b)) && charCheck chars a b
    charCheck [] _ _ = True
    charCheck (c:cs) a b = let zs = charmap M.! c in
                           (elem a zs ===> elem b zs) && charCheck cs a b

simulationQuotient :: Ord a => NFA a Char -> NFA a Char
simulationQuotient nfa = smQ nfa (smfun True nfa) (smfun False nfa)   
    where
    smfun b n = fu (stateSimulation b n) n
    fu x n = (nonTrivial em,em) where em = equivalenceMap x (states n)
    smQ n (False,_) (False,_) = n
    smQ n (True,em) _         = smQ nn (False,undefined) (smfun False nn)  where nn=nnfa em n
    smQ n (False,_) (True,em) = smQ nn (smfun True nn)   (False,undefined) where nn=nnfa em n
    mkfun ps = \x -> fromJust (lookup x ps)
    nnfa ps n = nfaTrans (mkfun ps) n

simulationQuotient2 nfa = smfun False True nfa
    where
    -- smfun TriedOtherDirection? direction nfa
    smfun s b n              = smfun2 s b (stateSimulation b n) n               -- computes simulation in b direction
    smfun2 s b sim n         = smfun3 s b sim n (equivalenceMap sim (states n)) -- computes simulation equivalence
    smfun3 s b sim n e       = smfun4 s (nonTrivial e) b sim n e                -- is sim equiv trivial?
    smfun4 s False b sim n e = smfun5 s b n (simulationPurgePar b sim n)        -- if trivial try purge redundant transitions
    smfun4 s True  b sim n e = smfun6 b sim (nnfa e n)                          -- if non-trivial, compute quotient
    smfun5 False b n Nothing = smfun True (not b) n                             -- direction b (now) fails, try other direction
    smfun5 True  _ n Nothing = n                                                -- tried everything, the end
    smfun5 _    b _ (Just m) = smfun False (not b) m                            -- trans were purged, bisim quotient, repeat
    smfun6 b sim n           = smfun5 True b n (simulationPurgePar b sim n)     -- try purge redundant transitions after quotient
    mkfun ps = \x -> fromJust (lookup x ps)
    nnfa ps n = nfaTrans (mkfun ps) n


simulationPurgePar :: Ord a=> Bool -> M.Map a [a] -> NFA a Char -> Maybe (NFA a Char)
simulationPurgePar direction mm nfa
    | length (trans nfa) == length ntrans
    = Nothing
    | otherwise
    = Just $ semanticStateMin $ removeUnreachable $ nfa { trans = ntrans }
    where
    ntrans = filter (not . redundant)(trans nfa)
    redundant (x,c,y) | direction
                      = any (\(nx,nc,ny)->nx==x && nc==c && ny/=y && simBy mm y ny)(trans nfa)
                      | otherwise
                      = any (\(nx,nc,ny)->nx/=x && nc==c && ny==y && simBy mm x nx)(trans nfa) 

simulationPurge nfa = removeUnreachable $ nfa { trans = filter (\p->not(fredundant p || bredundant p))(trans nfa) }
    where
    smf = stateSimulation True nfa
    smb = stateSimulation False nfa
    fredundant (x,c,y) = any (\(nx,nc,ny)->nx==x && nc==c && ny/=y && simBy smf y ny)(trans nfa)
    bredundant (x,c,y) = any (\(nx,nc,ny)->nx/=x && nc==c && ny==y && simBy smb x nx)(trans nfa)

-- applying a simulation
simBy :: Ord a => M.Map a [a] -> a -> a -> Bool
simBy mm x y = elem y (mm M.! x)


nonTrivial :: Eq a => [(a,a)] -> Bool
nonTrivial xs = any (uncurry (/=)) xs

equivalenceMap :: Ord a => M.Map a [a] -> [a]-> [(a,a)]
equivalenceMap fm ss = [ (x,y) | x<-ss, let y = head  [z | z <- fm M.! x, elem x (fm M.!z) ] ]

langComp :: Ord a => NFA a Char -> a -> a -> Ordering
langComp nfa x y = fst $ compSTinUF nfa emptyUF [x] [y]


compSTinUF :: Ord a => NFA a Char-> UF [a] -> [a] -> [a] -> (Ordering,UF [a])
compSTinUF nfa uf x y = solveGoalsUFSt nfa (makeGoalSt x y) uf

type GoalsST a = Queue ([a],[a])
makeGoalSt :: [a] -> [a] -> GoalsST a
makeGoalSt x y = singletonQ (x,y)

makeGoalsSt :: [[a]] -> GoalsST a
makeGoalsSt xs = list2Q (linkWith (,) xs)

type UFOrd a = (Ordering,UF [a])
solveGoalsUFSt :: Ord a => NFA a Char -> GoalsST a -> UF [a] -> UFOrd a
solveGoalsUFSt nfa gs = maybe successUFst (uncurry (compStsUF nfa)) (pollQM gs)

successUFst :: UF [a] -> UFOrd a
successUFst uf = (EQ,uf)

compStsUF :: Ord a => NFA a Char -> ([a],[a]) -> GoalsST a -> UF[a] -> UFOrd a
compStsUF nfa ([],[]) gs hyp = solveGoalsUFSt nfa gs hyp
compStsUF _   ([],_)  _  _   = (LT,emptyUF)
compStsUF _   (_ ,[]) _  _   = (GT,emptyUF)
compStsUF nfa (x,y) gs hyp   | f1 && not f2
                             = (GT,emptyUF)
                             | not f1 && f2
                             = (LT,emptyUF)
                             | b -- were already assumed to be the same
                             = solveGoalsUFSt nfa gs hyp'
                             | otherwise
                             = compStsUFpder nfa (stateSteps nfa x)(stateSteps nfa y) gs hyp'
                               where
                               (b,hyp')  =  unionTest x y hyp
                               f1 = isFinal nfa x
                               f2 = isFinal nfa y

compStsUFpder :: Ord a => NFA a Char -> [(Char,[a])] -> [(Char,[a])] -> GoalsST a -> UF[a] -> UFOrd a
compStsUFpder nfa [] [] gs = solveGoalsUFSt nfa gs
compStsUFpder nfa [] _  gs = resultUFSt LT gs
compStsUFpder nfa _  [] gs = resultUFSt GT gs
compStsUFpder nfa ((c,xs):rs) ((d,ys):ss) gs
    | c==d
    = compStsUFpder nfa rs ss (enterQ (xs,ys) gs)
    | c<d
    = resultUFSt GT gs
    | otherwise
    = resultUFSt LT gs

isFinal :: NFA a Char -> [a] -> Bool
isFinal nfa xs = any (final nfa) xs

resultUFSt :: Ordering -> GoalsST a -> UF [a] -> UFOrd a
resultUFSt ord goals hyp = (ord,emptyUF)

stateSteps :: (Ord a) => NFA a Char -> [a] -> [(Char,[a])]
stateSteps nfa xs = pairQuotient $ transitionList nfa xs

transitionList :: (Ord a,Ord b) => NFA a b -> [a] -> [(b,a)]
transitionList nfa xs = nubSort [ (c,x) | (a,c,x)<- trans nfa, elem a xs ]

pairQuotient :: (Ord a,Ord b) => [(b,a)] -> [(b,[a])]
pairQuotient [] = []
pairQuotient (p:xs) = addPQ p $ pairQuotient xs
                      where
                      addPQ (x,y) [] = [(x,[y])]
                      addPQ (x,y) ((z,zs):rs) | x==z
                                              = (z,y:zs) : rs
                                              | otherwise
                                              = (x,[y]):(z,zs):rs
